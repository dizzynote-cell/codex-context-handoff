# Auto Context Handoff for Codex/Codex自动上下文交接插件

对话太长时，提醒你换一个新对话，并帮你整理接着做所需的材料。

从插件记录的**第 4 次上下文压缩（大概50-80轮对话）**开始，每次压缩后提醒一次。你可以继续工作，也可以在之后任意一轮单独回复 `确认交接`。Codex 会整理当前目标、已完成工作、重要决定和下一步，生成交接文档与新对话的首条提示词。

> 上下文压缩，是 Codex 为容纳后续内容而浓缩历史对话的过程。这里统计的是压缩次数，不是聊天轮数；第 4 次是提醒规则，不代表任务已经出错。安装前的压缩不自动补计。

## 为什么用 Hook 和第 4 次压缩

语义偏移很难由正在工作的 AI 稳定自检：如果模型已经忘记早期限制或误解了目标，它也可能继续认为当前理解正确；等到用户纠正、文件冲突或测试失败时，偏移可能已经持续了一段时间。因此，本插件不把“AI 感觉自己是否偏了”作为主要触发条件，而是使用 Codex 实际发生的上下文压缩事件作为可观测、可复现的信号。

第 4 次阈值来自维护者对 Codex 5.6 及以上模型的长期实际使用经验：在所观察的开发任务中，前 4 次上下文压缩内几乎没有出现需要立即换对话的明显理解偏差。这个结论是选取默认阈值的经验依据，不是对所有模型、任务或用户的准确率保证。

冗余量也参考了一个约 200 多轮对话的长期任务：维护者在大约第 8～9 次上下文压缩时察觉到轻微的理解偏差，并认为即使只是轻微偏差，也不应继续依赖当前对话完成后续工作。因此从第 4 次开始提醒，意图是在轻微偏差出现前预留约 4～5 次压缩的交接窗口，而不是等偏差已经发生后再补救。这仍然是个案经验，不代表所有任务都会在相同位置发生偏移。

这种提前量也适合长时间运行的自动化或多 Agent 工作流：只要每个需要保护的 Codex 任务都启用了 Hook，并且编排流程会处理提醒和执行交接，就能降低主任务在无人察觉的情况下长期带着失真上下文继续运行的概率。它不能保证所有独立 Agent 或 Agent 间通信永远不出错，也不会在没有确认的情况下强制切换任务。

规则保持有意简单：

- 前 3 次压缩不打扰正常工作。
- 从第 4 次压缩开始，每次真实压缩后都提醒一次，不依赖模型主观判断。
- 不采用“提醒后再等待若干次压缩且由 AI 判断新阶段”的复杂冷却算法，避免漏掉已经进入风险区的对话。
- 提醒不代表任务已经出错，也不会自动交接；是否换对话始终由用户决定。

这种算法的优势不是预测语义偏移，而是在风险上升后提供一个确定、透明、容易验证的交接入口。

## 安装

需要支持本地插件及生命周期钩子（Hook）的 Codex，以及能通过 `python` 命令运行的 Python 3.11+。

```bash
codex plugin marketplace add dizzynote-cell/codex-context-handoff
codex plugin add context-handoff@context-handoff
```

安装后打开新任务，并按客户端提示审查、信任插件 Hook。Skill 和脚本已经包含在插件内，无需分别安装。

### 启用和检查 Hook

1. 安装插件后重新打开一个 Codex 任务。
2. 在 Codex CLI 中输入 `/hooks`。
3. 审查插件提供的 `PostCompact` 和 `UserPromptSubmit`，确认命令指向本插件目录后，将其标记为信任。
4. 确认两项均显示为已安装且已启用（`Installed 1`、`Active 1`）。
5. Hook 定义以后发生变化时，Codex 会要求重新审查；未获信任的 Hook 会被跳过。

部分图形客户端暂时没有完整的 Hook 管理入口，可以使用同一台机器上的 Codex CLI 完成审查。Windows PowerShell 如果阻止 `codex.ps1`，可以运行 `codex.cmd` 再输入 `/hooks`。

已知兼容情况：在 Windows Codex CLI 0.156.1 上，曾出现插件显示已安装、已启用，但 `/hooks` 中两项仍是 `0/0`。此时提醒不会运行，不能仅凭插件安装状态判断成功。可按[用户级 Hook 备用配置](docs/hook-fallback.md)接入脚本；插件自身的枚举问题仍待修复。

## 怎么用

1. 正常使用 Codex，等待交接提醒。
2. 想换对话时，单独发送：`确认交接`。
3. 插件生成交接文档和启动提示词。有兼容的创建接口时自动新建对话；否则，你新建对话并粘贴生成的提示词。

### 让安装前的老对话接入计数

插件不会自动猜测安装前发生过多少次压缩。对于仍要继续使用的老对话，可以在该对话里单独发送：

```text
初始化老对话压缩计数
```

只有当前 Codex 环境向 Hook 提供了该任务的本机会话记录路径，并且脚本拥有读取记录和写入插件状态的权限时，初始化才会执行。脚本只统计结构化的 `compacted` 事件，不根据轮数、摘要或模型感觉推算，也不接受提示词指定任意文件路径。初始化结果与当前监听计数取较大值而不是相加，所以重复执行不会重复累计；达到 4 次后立即进入可交接状态，后续继续由 `PostCompact` 监听。

如果会话记录未保存、权限不足或当前记录格式无法识别，插件会保留现有计数并显示初始化失败，不猜测历史次数。Codex 的会话记录格式不是稳定接口，因此该功能属于有明确失败提示的兼容能力。

交接以确认时的最新状态为准，保留未提交修改，不要求先提交代码或完成当前功能。材料保存在项目的 `.codex/handoffs/` 下：

若来源对话标题是「继续开发7」，下一对话会以「继续开发8」开头；交接文件的 `001` 仅是文件序号，不用于对话编号。兼容的外壳可根据来源会话 ID 把新对话归入原项目分类。

- `HANDOFF_001.md`：项目进度与背景，给接手对话阅读。
- `START_PROMPT_001.md`：粘贴到新对话的首条消息。
- `STATE_001.json`：插件进度记录，无需手动编辑。

## 需要配置吗？

**普通使用无需配置。** 默认尝试本机兼容接口，没有接口就生成材料供你手动新建对话。并不是所有 Codex 客户端或自制外壳都支持自动创建。

如果希望始终手动新建，在用户目录的 `.codex/context-handoff.toml` 中写入：

```toml
[handoff]
adapter = "manual"
```

Windows 路径示例：`C:/Users/你的用户名/.codex/context-handoff.toml`。也可以放进项目的 `.codex/context-handoff.toml`，仅对该项目生效，项目配置优先。

如果你使用**自制 Codex 外壳**（例如自己搭建的网页或手机访问界面），可以让插件调用它的创建对话接口。外壳需要先按协议接入，填写任意网址并不能直接生效。参见[外壳接入说明](docs/adapters.md)和[配置示例](plugins/context-handoff/config.example.toml)。

## Hook 能力与安全边界

Hook 会在本机执行公开的 Python 脚本，因此用户应当像审查其他本地自动化脚本一样，在启用前查看源码。Codex 不会因为安装插件就自动信任 Hook，用户可以通过 `/hooks` 审查或单独关闭它们。

- `PostCompact` 仅在上下文压缩后运行，用于增加当前任务的压缩计数并返回提醒。它保存会话标识、计数、时间、工作目录和少量去重信息，不保存完整聊天内容。
- `UserPromptSubmit` 会在每次提交提示词时启动，因为 Codex 当前不支持为该事件配置内容匹配器。脚本只判断输入在去除末尾标点和空白后是否严格等于 `确认交接`；普通提示词立即退出，不写入状态文件，也不发送到网络。
- 当输入严格等于 `初始化老对话压缩计数` 时，同一个 Hook 可以读取宿主提供的当前任务会话记录，补入安装前的结构化压缩事件。没有相应文件权限时不会执行。
- 两个 Hook 都不调用模型、不修改项目业务文件、不执行 Git 提交或部署，也没有额外遥测。
- 只有用户确认交接后，Skill 才会检查当前项目状态并在 `.codex/handoffs/` 写入交接材料。只有配置或检测到兼容适配器时，启动提示词才会交给该本机接口或程序。

这些限制使默认 Hook 的权限和数据范围较小，但任何本机脚本都不能笼统承诺“绝对安全”。源码、实际命令、安装来源及后续更新仍应由用户审查。详细说明见[安全说明](SECURITY.md)。

## 适用范围与数据

自动提醒取决于运行 Codex 的机器是否支持并启用了 Hook。用手机访问自制网页也可以，前提是后端支持；仅安装到不运行本地脚本的网页环境不会获得自动提醒。

计数保存在本机，插件没有额外遥测。自动创建对话时，启动提示词会交给所选接口或程序，再由它发送给 Codex。默认模式会探测本机接口；如不希望使用它，设置 `manual`。

目前完成了 Windows 上的脚本模拟与结构校验；跨平台兼容性和各客户端实际提醒显示仍需进一步验证。

[English](docs/README.en.md) · [贡献指南](CONTRIBUTING.md) · [安全说明](SECURITY.md) · [MIT 许可证](plugins/context-handoff/LICENSE)
